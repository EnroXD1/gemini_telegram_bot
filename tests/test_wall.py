...
@pytest.mark.parametrize("count", WALL_PART_COUNTS)
def test_media_groups_are_within_telegram_limits(count: int) -> None:
    paths = tuple(Path(f"{index}.jpg") for index in range(count))

    batches = media_group_batches(paths)

    assert sum(len(batch) for batch in batches) == count
    assert all(2 <= len(batch) <= 10 for batch in batches)
    ...
    class FakeAnchor:
        ...
        async def answer_media_group(self, *, media: list[Any]) -> None:
            for item in media:
                ...
                assert path.exists()
                observed_paths.append(path)
                sent_names.append(item.media.filename)
